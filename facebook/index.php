...
foreach( $fqlResult as $keys => $values ){
	...
	$start_date = substr($values['start_time'] , 0, 10 );
	$start_time = substr($values['start_time'] , 11, 5);
...
echo "<div class='event'>";
	...
	$spos = strpos($beskrivelse, "'");
	$beskrivelse = substr($beskrivelse, $spos+1, strlen($beskrivelse));
	$spos = strpos($beskrivelse, "'");
	$removed = substr($beskrivelse, 0, $spos);
	$beskrivelse = substr($beskrivelse, strlen($removed)+1, strlen($beskrivelse));*/
	$pos = strpos($beskrivelse, '.');
    $ingress = substr($beskrivelse, 0, $pos+1);
	$ingress = trim($ingress);
	$beskrivelse = substr($beskrivelse, $pos+1, strlen($beskrivelse));
	if ($values['name']){
	echo "<b>{$values['name']}</b>";
	...
	echo "<b>Foredrag arrangert av Verdi NTNU</b>";
	echo "<br>";
	echo "<i>Verdi NTNU arrangerer flere foredrag hvert semester, vi inviterer spennende foredragsholdere til å snakke om det de kan best. Det vil komme informasjon om
	neste foredrag her</i>";}
