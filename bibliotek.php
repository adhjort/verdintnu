...
// That way everyone can see the page but username and  are needed for uploading.
// For example: $_CONFIG['users'] = array(array("username", "password", "admin"));
// Default: $_CONFIG['users'] = array();
//

$username = $_SESSION['info']['brukernavn'];
$aar = $_SESSION['info']['avgangsaar'];
$_CONFIG['users'] = array(array("verdi", "verdintnu", "admin"));
//
...
if ((strpos($queryStr, 'logout') !== false)){
	echo "<meta http-equiv='refresh' content='0;url=http://verdintnu.no'>";
	session_unset();
	session_destroy();
}
?>
